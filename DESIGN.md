...
- **Wordmark:** "O<small>NE</small>V<small>OICE</small>**27**" in italic Figtree 900 (1.4rem), set like the official logo: O and V are full-size capitals, "ne" and "oice" are small caps, and 27 is full height in Candle Pink. Figtree has no real small-caps glyphs (no `smcp` feature), so never use `font-variant: small-caps`; the small caps are `<span class="sc">` capitals at 0.78em on the same baseline. The visible name is `aria-hidden` and the link's `aria-label` reads "OneVoice27 Portugal, início". A teal "PORTUGAL" tag (0.7rem, 0.2em tracking) is centred beneath the name, like the reference logo's tagline.
- **Tabs:** a segmented pill track on `night-0`. The selected tab is shown by **one sliding indicator**: a copy of the tab row with `night-0` labels on the role tone, laid exactly over the real tabs and clipped with `clip-path: inset(... round 999px)` (12px when stacked) to the selected tab. Switching tabs animates the clip insets and the colour (480ms, ease-out-quint). Never animate width or left: the clip keeps the pill ends round mid-flight, and the label copy keeps every label readable while the pill passes under it. It snaps without animating on first paint and on resize. The newly shown form fades up 10px (420ms). All of this stops under reduced motion. Arrow keys, Home and End move between tabs, and the choice syncs to `#instrutor`, `#criador` or `#igreja`. Below 560px the tabs stack.
