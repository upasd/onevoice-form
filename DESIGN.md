...
- **Light halo** (`box-shadow: 0 0 24px 4px color-mix(in oklch, var(--tone) 45%, transparent)`): path step lights only. The map and confirmation lights use radial gradients instead of shadows.
...
Four steps joined by one gradient thread: horizontal on desktop (column widths 1.15 / 0.7 / 1.15 / 1.15) and vertical below 960px. Role steps have a 27px glowing tone light, a phase label, a title, copy and an underlined arrow link. Step 2 ("Apelo") is the interested person: a smaller white light and an italic quote, with no call to action.
